...
public class Ejecutable implements ActionListener, WindowListener {
	...
	Panel pnlWpn = new Panel();
	Panel pnlArmr = new Panel();
	...
	Label lblWinPrbs = new Label("Has destrozado al muñeco de Pruebas");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == btnStats) {
			int armaEleg = wpnList.getSelectedIndex();
			int armaduraEleg = armrList.getSelectedIndex();
			Arma arma = armas[armaEleg];
			Armadura armadura = armaduras[armaduraEleg];
			txtInfo.setText("ARMA: " + arma.getNombre() + " // DAÑO:  " + arma.getMindmg() + " ~ " + arma.getMaxdmg() + "  // DURABILIDAD: " + arma.getDurabilidad() +
					"\nARMADURA: " + armadura.getNombre() + " // RESISTENCIA:  " + armadura.getResistencia() + " // DURABILIDAD: " + armadura.getDurabilidad());	
		}
					...
					if (dmg < armadura.getResistencia()) {
						...
						txtInfo.setText("Vas a realizar un ataque con un/una \"" + arma.getNombre() + "\" a un/una \""
								...
								+ "\nEl arma realiza un golpe de " + dmg + " ." + "\nEl arma ha atravesado la armadura."
										+ "\nDurabilidad " + arma.getNombre() + " : " + arma.getDurabilidad()
										+ "\nDurabilidad " + armadura.getNombre() + " : " + armadura.getDurabilidad());
					}
				} else {
